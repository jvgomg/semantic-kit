...
describe('buildSocialPreview', () => {
  describe('title fallback', () => {
    it('uses twitter:title when present', () => {
      const result = buildSocialPreview(
        { 'og:title': 'OG Title' },
...
describe('buildPreview (legacy wrapper)', () => {
  it('works with SocialTags format', () => {
    const tags: SocialTags = {
      openGraph: { 'og:title': 'OG Title' },
      twitter: { 'twitter:title': 'Twitter Title' },
    }
    const result = buildPreview(tags, emptyPageMetadata, 'https://example.com')
    expect(result.title).toBe('Twitter Title')
  })
