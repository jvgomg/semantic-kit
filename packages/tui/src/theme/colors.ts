...
/**
 * Build dimmed colors from resolved colors.
 * Used when content is behind a modal.
 */
export function buildDimmedColors(
  colors: ResolvedColors,
  palette: AnsiPalette = DEFAULT_ANSI_PALETTE,
): ResolvedColors {
  const muted = colors.textMuted
  const veryMuted = palette[ANSI.BRIGHT_BLACK]
  const subtle = palette[ANSI.BLACK]

  // Define overrides for core colors
  const overrides: Partial<Record<ThemeColorKey, string>> = {
    // Text - dim everything
    text: muted,
    textMuted: veryMuted,
    ...
    // Borders - more subtle
    border: veryMuted,
    borderActive: veryMuted,
    borderSubtle: subtle,
    borderSelected: veryMuted,
    ...
    // Markdown - dimmed
    markdownHeading: muted,
    markdownLink: muted,
    markdownCode: muted,
    markdownBlockQuote: veryMuted,
    markdownList: muted,

    // Syntax - dimmed
    syntaxComment: veryMuted,
    syntaxKeyword: muted,
    syntaxFunction: muted,
    syntaxString: muted,
    syntaxNumber: muted,
    syntaxType: muted,
  }

  // Build the dimmed color object
  const dimmed = {} as Record<string, string>
  for (const key of THEME_COLOR_KEYS) {
    dimmed[key] = overrides[key] ?? colors[key]
  }

  return dimmed as ResolvedColors
}
